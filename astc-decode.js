...
function decodeASTCTexture(astcData) {
    ...
    try {
        ...
        if (paddedWidth !== header.width || paddedHeight !== header.height) {
            const properSizedData = new Uint8Array(trueSize);
            for (let y = 0; y < header.height; y++) {
                const targetRowStart = y * header.width * 4;
                const sourceRowStart = y * paddedWidth * 4;
                const rowLength = header.width * 4;
                properSizedData.set(
                    decodedImage.subarray(sourceRowStart, sourceRowStart + rowLength),
                    targetRowStart
                );
            }
        ...
        if (decodedImage.length > trueSize) {
            return decodedImage.subarray(0, trueSize);
        }
